...
/// <summary>
/// PriorityQueue class based on Microsoft implementation from .NET Framework 4.8. Unlike the implementation from .net Core it doesn't
/// require a separate priority value and relies on the provided comparer.
/// </summary>
public class PriorityQueue<T>
{
    ...
    public PriorityQueue(int capacity = DefaultCapacity, IComparer<T> comparer = null)
    {
    ...
    internal T Pop()
    {
        ...
        if (_count > 0)
        {
            ...
            T res = _heap[0]; // save the root for return
    ...
    internal bool Remove(T item)
    {
        ...
        int index = Array.IndexOf(_heap, item, 0, _count);
        if (index < 0)
        {
            return false;
        }
        --_count;
        // Move the last item into the gap left by the removed item.
        T x = _heap[_count];
        _heap[_count] = default; // don't leak x
        // Sift down or up to restore the heap property.
        if (index < _count)
        {
            if (_isHeap)
            {
